...
export default function Header() {
  ...
  const visibleLinks = user ? [[base, 'Dashboard']] : links
  const logout = () => {
    signOutStaff()
    setOpen(false)
    ...
    <header className="site-header">
      <div className="shell header-inner">
        ...
        <nav id="site-nav" className={open ? 'site-nav site-nav--open' : 'site-nav'} aria-label="Main navigation">
          {visibleLinks.map(([path, label]) => <NavLink key={path} to={path} end={path === base || path === '/'} onClick={() => setOpen(false)} className={({ isActive }) => isActive ? 'active' : ''}>{label}</NavLink>)}
